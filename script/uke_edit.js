...
window.onload = function main() {
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      thumbnail.src = host_url+"/uke_blog/data/"+song_data.thumbnail;
...
*/
      ...
      ////  Drawing Tabulature
      resize_canvas( window.innerWidth-60);
    }
  };
...
// var zoom_factor = 10;     // samples_per_pixel;
// var samples_per_quaver = zoom_factor*80;     // samples_per_quaver;
var g_sampleRate = 0;
var g_totalSec = 100
var numSmp_pixel = 16;     // number of samples per pixel;        --> 1, 2, 4, 8, 16, 32, 64, 128, .. <-- 확대/축소 배율
var numSmp_quaver = 320;    // number of samples per quaver (8분음표)
...
function draw_tabulature() {
  ...
  if (song_data.source) {     // mp3 데이터가 있는 경우에만 표시.
    ...
    audioTag.src = host_url+"/uke_blog/data/"+song_data.source;

    var oReq = new XMLHttpRequest();
    oReq.open("GET", host_url+"/uke_blog/data/"+song_data.source, true);
    ...
    oReq.onload = function() {      //     // oReq.onreadystatechange = function() {
      ...
      //  drawWaveBackground(ctx);
       if (array_l) {
          ...
          mp3Draw(array_l);   //        const float32Array_left = audioBuf.getChannelData(0);
       }
  ...
  async function mp3Decode(arrayBuffer) {
    ...
    // if (array_r) {
    //   mp3Draw(array_r);   //        const float32Array_left = audioBuf.getChannelData(1);
    // }
  }
...
function drawWaveBackground(ctx) {
  console.log("drawWaveBackground:");
  // ctx.strokeStyle = "rgb(255,255,128)"; //"red";
  var bpm = document.getElementById("bpm").value;
  var offset = document.getElementById("offset").value;

  numSmp_quaver = g_sampleRate / bpm;

  var grid_width = (numSmp_quaver/numSmp_pixel)/10;
  var beat_count = 0;
  console.log("BPM="+bpm+", sampleRate="+g_sampleRate+", numSmp_Quaver="+ numSmp_quaver+", gridWidth="+grid_width+", numSmp_pixel="+numSmp_pixel );
  for (var i = 0; i< canvas_width; i+=grid_width ) {
      ctx.fillStyle = "lightgray";
      ctx.fillRect(i, 0, 1, 200);
      if ((beat_count%8)==0) {
          ctx.fillStyle = "rgb(128,255,240)";
      } else {
          ctx.fillStyle = "rgb(192,255,248)";
      }

      if (( (i*numSmp_pixel)%g_sampleRate)==0) {
        ctx.fillStyle = "red";
      }

      beat_count++;       // 박자에 따른,  1개 마디별 음표 갯수 count
      ctx.fillRect(i+1, 0, grid_width-1, 200);
  }
  
}

/*
function mp3Draw(arrayBuffer, ctx) {
  ...
  for (var i = 0; i< canvas_width; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo( i+0.5, 100.5 - min );
    ctx.lineTo( i+0.5, 100.5 + max );
    ctx.stroke();
...
var changeThumnail = () => {    /* when ThumbNail file upload succed. */
  console.log("Tell me something " );
  var selectedFiles = document.getElementById("loadThumbnail");
  var imgTag = document.getElementById("thumbnail");
  imgTag.src = selectedFiles.files[0].name;
  console.log("selectedFiles:" + selectedFiles.files[0].name );
}
