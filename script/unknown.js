...
var draw_waveform = (ctx, ypos, height) => {
  ...
  if (array_l && array_l.length>0) {    // MP3 디코딩 된 데이터가 있으면 그린다. 없으면 안그림.
    ...
    // draw_mp3Wav(0, array_l);    // a new wave form drawing ..
  } else {
...
var waveformDraw = (ctx, ypos, wavBuffer) => {
  ...
  for ( i = 0; i< (canvas_width-START_XPOS); i++ ) {
    ...
    } else if ( (g_selection_start > temp_index) && (g_selection_end < temp_index) ) {  // selected area
      ctx.fillStyle = 'yellow';
      ctx.fillRect(START_XPOS+i+0.5, ypos, 1, editor_height);
    } else if ( Math.abs(focused_msec-temp_index) < g_numSmp_per_px) {
      ctx.fillStyle = 'red';
      ctx.fillRect(START_XPOS+i+0.5, ypos, 1, editor_height);
